...
exports.run = async (client, message, args) => {
  ...
  if(![("Setting.Roles.yetkili")].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) return message.reply(ravi2.setDescription("<a:9a:920095225081372762> **Bu Komut İçin Yetkiniz Bulunmamaktadır.**"))
...
users.roles.remove(r.id)
  ...
  message.channel.send(embed.setDescription(`${users} Adlı Kullanıcı Başarıyla Kayıtsız'a Atıldı.`).setTimestamp().setColor("RED").setFooter("Arvelos 1672"))
}
module.exports.conf = {
  enabled: true,
  guildOnly: true,
